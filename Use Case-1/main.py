...
# Data Model for TestReport (mirrors Prisma schema)
  # Or handle in DB if Prisma manages it
class TestReport(BaseModel):
    ...
    mrUnused: int
    mrBugs: int
    createdAt: datetime.datetime  # Or handle in DB if Prisma manages it
    
# Sample Data (replace with your actual data loading)
# This simulates weekly data, enough to form monthly trends
sample_data = [
...
@app.post("/predict")
async def predict_next_month(data: List[TestReport]): # Takes a list of TestReport objects
    """
    ...
    try:
        ...
        logger.debug("DataFrame created: %s", df.head()) # Log the first few rows of the DataFrame
        df['createdAt'] = pd.to_datetime(df['createdAt'])  #Crucial: Convert string dates to datetime
        logger.debug("createdAt converted to datetime")
        df = df.sort_values(by='createdAt')
        logger.debug("Data sorted by createdAt")
        # Simple Linear Regression (for demonstration).  Expand this!
        X = df[['storyTests', 'regressionTestsAutomated', 'regressionTestsManual']] # Features
        y = df['totalTestsByApplication']  # Target
        logger.debug("Features and target defined")
        logger.debug("X: %s", X)
        logger.debug("y: %s", y)
        #Handle potential missing data (very basic)
        X = X.fillna(X.mean())
        y = y.fillna(y.mean())
        ...
        last_week_data = X.iloc[[-1]]  #DataFrame containing features from last week
        logger.debug("Last week data: %s", last_week_data)
        # Predict for next month using the last week's input features
        next_month_prediction = model.predict(last_week_data)[0]  # Extract single prediction
        logger.debug("Next month prediction: %s", next_month_prediction)

        # Create dummy data for the next month to be added to the model
        ...
        next_month_data = {
            ...
            'storyTests': int(last_week_data['storyTests'].iloc[-1] * 1.1),  # Example trend
        ...
        # Convert the dictionary to a TestReport object
        next_month_report = TestReport(**next_month_data)
        logger.debug("Next month report created: %s", next_month_report)
        return {"prediction": int(next_month_prediction), "next_month_data": next_month_report} # Convert to int for cleaner output
