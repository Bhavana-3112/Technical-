...
INCREASE_FACTOR = 1.05  # For dummy data generation
...
async def predict_next_month(data: List[TestReport]):
    ...
    try:
        ...
        df = df[features + ['createdAt']]  # Include createdAt for time series ordering
        ...
        scaled_data = scaler.transform(df)  # Use the loaded or fitted scaler
        ...
        def create_dataset(dataset, time_step=1):
            ...
            for i in range(len(dataset) - time_step - 1):
                ...
                X.append(a)
                y.append(dataset[i + time_step, 3])  # Predict 'totalTestsByApplication'
            return np.array(X), np.array(y)

        time_step = 3  # Look back at the previous 3 weeks
        X, y = create_dataset(scaled_data, time_step)
        X = X.reshape(X.shape[0], X.shape[1], X.shape[2])  # reshape for LSTM

        # Split into train and test sets (Manual Split for Time Series)
        ...
        if os.path.exists(MODEL_PATH):
            ...
            # Train the model
            model.fit(X_train, y_train, epochs=50, batch_size=1, verbose=0)  # train silently
        ...
        # Create the next month's data using some assumptions.  This is where you'd inject more sophisticated forecasting.
        next_month_data = {
            'id': int(data[-1].id + 1),
            'projectName': data[-1].projectName,
            'authors': data[-1].authors,
            'storyTests': int(data[-1].storyTests * INCREASE_FACTOR),  # Example: 5% increase
            ...
            'storyUnexecuted': int(data[-1].storyUnexecuted * (1 + (1 - INCREASE_FACTOR))),  # Example: Decrease
            'storyBlocked': int(data[-1].storyBlocked * (1 + (INCREASE_FACTOR - 1))),  # Example: Increase
        ...
        return {"prediction": int(next_month_prediction),
                ...
                "mae": mae,  # Include evaluation metrics in the response
                "rmse": rmse}
